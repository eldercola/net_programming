...
timeout.tv_usec = 0;
 ...
 if(listen(listenfd, BACKLOG) == -1)
 {
  ...
  while(1){
    ...
    printf("--------------A client begins--------------\n");
    //Assign the address and port to the client_address
    ...
    while(1){
      ...
      if(strcmp(rvmsg, "OK\n")==0){//server receives ok
        ...
        initCalcLib();
        operate = randomType();
        //randomize the content according to the type of operator
        if(operate[0] == 'f'){
          float f1,f2;
          f1 = randomFloat();
          f2 = randomFloat();
          ...
          val1 = randomInt();
          val2 = randomInt();
          //val 2 couldn't be 0 when handling div
        ...
        if(fabs(remoteResult-opeResult) < 0.0001){
          if(send(connfd, "OK\n", sizeof("OK\n"), 0) == -1){
            perror("send OK");
            break;
          }
